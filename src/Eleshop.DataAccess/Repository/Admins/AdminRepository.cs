...
namespace Eleshop.DataAccess.Repository.Admins;

public class AdminRepository : BaseRepositories, IAdminRepository
{
    ...
    public async Task<int> DeleteAsync(long id)
    {
        ...
        {
            await _connection.OpenAsync();
            string query = "DELETE FROM public.users WHERE role=1 and id=@Id;";
    ...
    public async Task<UserViewModel?> GetByIdAsync(long id)
    {
        ...
        {
            await _connection.OpenAsync();
            string query = "Select * From users where id=@Id and role=1";
    ...
    public async Task<User?> GetByPhoneAsync(string phone)
    {
        ...
        {
            await _connection.OpenAsync();
            string query = "SELECT * FROM users WHERE phone_number = @PhoneNumber and role = 1";
    ...
    public async Task<(long ItemsCount, IList<UserViewModel>)> SearchAsync(string search, PaginationParams @params)
    {
        try
        {
            ...
            string query = $"Select * From users Where role = 1 and first_name ILIKE @search Order By first_name " +
                $"Offset {@params.GetSkipCount()} Limit {@params.PageSize}";
