...
namespace Eleshop.DataAccess.Repository.Users;
...
public class UserRepository : BaseRepositories, IUserRepository
{
    ...
    public async Task<int> DeleteAsync(long id)
    {
        ...
        {
            await _connection.OpenAsync();
            string query = "DELETE FROM public.users role=0 and WHERE id=@Id;";
    ...
    public async Task<(long ItemsCount, IList<UserViewModel>)> SearchAsync(string search, PaginationParams @params)
    {
        try
        {
            ...
            string query = $"Select * From users Where role = 0 and first_name ILIKE @search Order By first_name " +
                $"Offset {@params.GetSkipCount()} Limit {@params.PageSize}";
